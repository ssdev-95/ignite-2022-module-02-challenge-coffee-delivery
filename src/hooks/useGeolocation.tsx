...
export function GeoLocationProvider({
	...
	async function getGeolocation(coords:Coordinates) {
		...
		const url = `/revgeocode?at=${coords.latitude},${coords.longitude}&apiKey=${import.meta.env.VITE_API_KEY}`
		const { data } = await api.get<GeoResponse>(url)
			.catch((err) => {
				alert(err)
			})
		...
		localStorage.setItem(
			storeKey,
			JSON.stringify(data.items[0].address)
		)
